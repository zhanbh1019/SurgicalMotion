...
from util import normalize_coords
...
class BaseTrainer():
    ...
    def calculate_mask_loss(self,mask_1, mask_rigid):
        
        indices = mask_1 > 200
        targets = mask_1[indices] / 255.0
        predictions = mask_rigid[indices] / 255.0
        loss = F.mse_loss(predictions, targets)
        
        return loss
    
    def ARAP_loss(self, x1s_pred, x2s_pred, mask_1):
        mask_1 = (mask_1 > 200)
        loss = 0
        num = 0
        
        for frame in range(mask_1.shape[0]):
            mask = mask_1[frame]
            if mask.sum() < 2:
                continue

            x1 = x1s_pred[frame][mask]
            x2 = x2s_pred[frame][mask]

            vec = x1 - x2
            
            vec_x1_x2 = torch.cat((vec, x1, x2), dim=1)
            vec_x1_x2 = vec_x1_x2.cpu().detach().numpy()
            if len(vec) >= 2:
                kmeans = KMeans(n_clusters=2, random_state=0, n_init=10)
                labels = kmeans.fit_predict(vec_x1_x2)
                
                for label in set(labels):
                    indices = (labels == label)
                    
                    x1_cluster = x1[indices].cpu().detach().numpy()
                    x2_cluster = x2[indices].cpu().detach().numpy()

                    if len(x1_cluster) > 1:
                        diff_x1_x2 = np.abs(x1_cluster - x2_cluster)
                        diff_matrix = np.linalg.norm(diff_x1_x2[:, None, :] - diff_x1_x2[None, :, :], axis=-1)

                        i_upper = np.triu_indices_from(diff_matrix, k=1)
                        loss += diff_matrix[i_upper].sum()
                        num += len(diff_matrix[i_upper])
    ...
    def compute_all_losses(self,
        ...
        mask_2 = batch['mask_2'].to(self.device)
        ...
        px2s_proj_flow = px2s_proj[:,0:px2s_proj.shape[1] * 3 // 4]
        px2s_flow = px2s[:,0:px2s.shape[1] * 3 // 4]
        weights_flow = weights[:,0:weights.shape[1] * 3 // 4]
        flow_mask = flow_mask | mask[:,0:mask.shape[1] * 3 // 4]

        px2s_proj_sparse = px2s_proj[:,px2s_proj.shape[1] * 3 // 4:]
        px2s_sparse = px2s[:,px2s.shape[1] * 3 // 4:]
        weights_sparse = weights[:,weights.shape[1] * 3 // 4:]
        mask_sparse = mask[:,mask.shape[1] * 3 // 4:]

        px2s_proj_normed = normalize_coords(px2s_proj, self.h, self.w)[None]
        mask_rigid = F.grid_sample(mask2[None].float().permute(1,0,2,3), px2s_proj_normed.permute(1,0,2,3),
        ...
        if mask.sum() > 0:
            ...
            optical_flow_loss = masked_l1_loss(px2s_proj_flow[flow_mask], px2s_flow[flow_mask], weights_flow[flow_mask], normalize=False)
            optical_flow_grad_loss = self.gradient_loss(px2s_proj_flow[flow_mask], px2s_flow[flow_mask], weights_flow[flow_mask])

            mask_loss = self.calculate_mask_loss(mask_1[mask], mask_rigid[mask])
            arap_loss = self.ARAP_loss(x1s_pred, x2s_pred, mask_1)

            long_term_loss = masked_l1_loss(px2s_proj_sparse[mask_sparse], px2s_sparse[mask_sparse], weights_sparse[mask_sparse], normalize=False)

        else:
            loss_rgb = loss_rgb_grad = optical_flow_loss = optical_flow_grad_loss = mask_loss = arap_loss = long_term_loss = torch.tensor(0.)
        ...
        loss = optical_flow_loss + \
               ...
               w_mask * mask_loss +\
               w_arap *arap_loss  +\
               w_long_term_loss * long_term_loss
        ...
        if write_logs:
            ...
            self.scalars_to_log['{}/loss_long_term_flow'.format(log_prefix)] = long_term_loss.item()
            self.scalars_to_log['{}/loss_mask'.format(log_prefix)] = mask_loss.item()
            self.scalars_to_log['{}/loss_arap'.format(log_prefix)] = arap_loss
    ...
    def train_one_step(self, step, batch):
        ...
        w_mask = self.weight_scheduler(step, 20000, 1, 0, 1)
        w_long_term_loss=0.3
        w_arap = self.weight_scheduler(step, 20000, 1, 0, 1)
        ...
        loss, flow_data = self.compute_all_losses(batch,
                                                  ...
                                                  w_mask = w_mask,
                                                  w_arap = w_arap,
                                                  w_long_term_loss=w_long_term_loss,
                                                  return_data=True)
